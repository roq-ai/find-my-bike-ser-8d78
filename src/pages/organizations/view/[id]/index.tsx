...
import { deleteBikeRepairStationById } from 'apiSdk/bike-repair-stations';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'bike_repair_station'],
      }),
  );

  const bike_repair_stationHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('bike_repair_station', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/bike-repair-stations/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.bike_repair_station?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/bike-repair-stations/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/bike-repair-stations/edit/${record.id}`}
                                >
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    bike_repair_stationHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
