...
import { getBikeRepairStationById, updateBikeRepairStationById } from 'apiSdk/bike-repair-stations';
...
function BikeRepairStationEditPage() {
  ...
  const handleSubmit = async (values: BikeRepairStationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBikeRepairStationById(id, values);
      mutate(updated);
      resetForm();
