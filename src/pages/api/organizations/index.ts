...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.bike_repair_station?.length > 0) {
      const create_bike_repair_station = body.bike_repair_station;
      body.bike_repair_station = {
        create: create_bike_repair_station,
      };
    } else {
      delete body.bike_repair_station;
    }
